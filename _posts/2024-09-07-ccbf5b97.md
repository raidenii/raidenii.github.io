...
In my case, `ttyUSB2` is the AT command interface, YMMV. Just try each one out if USB2 is not, it won't hurt. Use 

```
socat /dev/ttyUSB2,b115200,echo=0,crnl -
```

 to talk to the modem, start with `ATI`, you should see something like:
...
To change outgoing TTL, enable ADB first. Get the key:
...
Then use a Python script to calculate the unlock key:
...
The put it back to the serial:

```
AT+QADBKEY="put-unlock-key-here"
```

Then you can use adb from openwrt to access the modem. You can use vi to edit, but for the sake of convenience, just pull the systemd unit file and script from [this git repo](https://github.com/natecarlson/quectel-rgmii-configuration-notes).

Run the following to install the script and systemd:
...
Now the modem will automatically set all outgoing TTL/HL to 64.
...
to update firmware. It appears that openwrt 24.10 x86 no longer offers `qfirehose` from its repo, you'll need it from [here](https://forumschinese.quectel.com/t/topic/7168).